...
public class TestApplicationMemory {

    public static void main(String[] args) {
        //SpringApplication.run(TestArnaudApplication.class, args);

        ConfigurableApplicationContext context = SpringApplication.run(TestApplicationMemory.class);
        ClientRepository repository = context.getBean(ClientRepository.class);
        // save a couple of Clients
        repository.save(new Client("Bauer", "Jack"));
        repository.save(new Client("Chloe", "O'Brian"));
        repository.save(new Client("Bauer", "Opi"));
